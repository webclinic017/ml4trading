...
import hca_segmsec_config as scfg
...
## Write Portfolio Summary File(PSF) for Bot communication.
#with open(PSF_name, 'w') as f:
    #tmpstring = ""    
    #for key, value in context.record.items():
...
###################### Global Constants ###################################
#
def find_latest_trade(daily_port_date_traded_file, context): # Find out if bundle name needs an update now.   

    traded_df=pd.DataFrame()
    today        = datetime.today() #Current day live
    #today = context.datetime.now(tz='US/Eastern')    
    brk_acct_bo =context.broker.get_account_from_broker()
    pp.pprint("IB-account:find_latest_trade:{} {}".format(today, brk_acct_bo))

    today_netliq = float(brk_acct_bo['NetLiquidation'])   
    prev_netliq  = float(brk_acct_bo['PreviousDayEquityWithLoanValue'])
                        
    daily_pnl    = 1.0 -(today_netliq / prev_netliq)

    pp.pprint(f'today:{today} ')
    pp.pprint(f'today_netliq:{today_netliq:.2f} ')
    pp.pprint(f'prvday_netliq:{prev_netliq:.2f} ')
    pp.pprint(f'pnl_pct:{daily_pnl*100:.2f} ')
     
    try:
        traded_df = traded_df.from_csv(daily_port_date_traded_file)
    except:
        log.info(f"{daily_port_date_traded_file} does not exist. Creating it")
        col_names  = ['OrdersDone', 'netliq', 'prvnetliq', 'dailypnl']
        #traded_df  = pd.DataFrame(data=[context.ORDERS_DONE], columns=col_names, index=[today], dtype=col_dtypes)
        traded_df  = pd.DataFrame(data=[], columns=col_names, index=[today])
        col_dtypes = {col_names[0]: 'bool',  col_names[1]: 'float',  col_names[2]: 'float',  col_names[3]: 'float'}
        data_df = traded_df.astype(dtype=col_dtypes)
        traded_df.index.name = 'Time'
        traded_df.OrdersDone =context.ORDERS_DONE
        (traded_df.to_csv(daily_port_date_traded_file))
        
    if len(traded_df) > 0:
        last_trade_row   = traded_df.tail(1)
        time_last_traded = last_trade_row.index
        traded_delta_time =  today - time_last_traded
        
        if last_trade_row['OrdersDone'][0]:
            log.info(f"Policy: Already Ordered Today {last_trade_row}")
        else:
            log.info(f"Policy: Ordered Today {last_trade_row}")
            
        # (find out if today's trade is already done.)
        ##delta_time      = pd.datetime.now() - bundle_latest_dt
        delta_time      = traded_delta_time
        delta_days      = delta_time.days
        delta_secs      = delta_time.total_seconds()
        delta_hrs       = delta_secs/float((60*60))
        ###orig do_update_bundle= (delta_days > 0) | (delta_secs > ((60*60)*6.5)) # Trading Day is 6.5 hours long, 
                                                                          #  so refresh if more than a TradingDay since last refresh.
        orders_done_today = bool((delta_days <= 0) and  last_trade_row['OrdersDone'][0])
        #| (delta_secs > ((60*60)*6.5)) # refresh only if day is wrong
        print("orders_done_today={}, delta_days={},delta_hrs={} delta_secs={}".format(orders_done_today, delta_days, delta_hrs, delta_secs))

        # Update new pnl row with orders_done_today flag plus live broker pnl values as of NOW.
        #line = pd.to_datetime("2014-05-01 18:50:05", format="%Y-%m-%d %H:%M:%S")
        new_row = pd.DataFrame(data=[[orders_done_today, today_netliq,prev_netliq, daily_pnl*100]], columns=traded_df.columns, index=[today])
        traded_df = pd.concat([traded_df, pd.DataFrame(new_row)], ignore_index=False)
        (traded_df.to_csv(daily_port_date_traded_file)) #Write streaming portfolio info to portfolio file.
        
    return traded_df, orders_done_today
...
class OrderMngr(object):
    ...
    def process_order_queue(self, context,  data):
        ...
        for sec, amount in list(self.order_queue.items()):
            ...
            if (sec.symbol == 'TWOU') or (pct_actual > 0.14):
                log.info("sec[{}]: pct_actual={}   amt={} price={}".format(sec, pct_actual, amount, price[sec]))
        ...
        if today_short_cash > 0:
            ...
            for sec in self.shorts.index:
                try:
                    ...
                    amount = 0
                    log.info("BAD_Short_Order_AMT:{}".format(sec))
        ...
        if today_long_cash > 0:
            ...
            for sec in self.longs.index:
                try:
                    ...
                    amount = 0
                    log.info("BAD_Long_Order_AMT:{}".format(sec))
                    
                new_positions[sec] = round(amount)

        # save daily positions so that next time we know what we have to rebalance
        #self.days[self.curr_day] = self.context.exposure.self.long_actual_dist.update(self.context.exposure.self.short_actual_dist)
...
def initialize(context):
    ...
    #2
    s1 = Strategy(rebalance_days=1, max_long_sec=300, max_short_sec=300,
    ...
    if not AlgoParms['IS_LIVE']: # Run Backtest Simulation
        ...
        schedule_function(cancel_open_orders, date_rules.week_start(days_offset=4), time_rules.market_close())

        #### Daily
        #######schedule_function(rebalance, date_rules.every_day(),time_rule=time_rules.every_minute())
        #for i in range(1, 391):
        #   schedule_function(rebalance, date_rules.every_day(), time_rules.market_open(minutes=i))
        ###DEBUG:start:ajjc
        #for i in range(1, 391):
    ...
    #schedule_function(rebalance, date_rules.every_day(),time_rule=time_rules.market_open(minutes=30))
    ###schedule_function(rebalance, date_rules.every_day(),time_rule=time_rules.market_close(minutes=65))
    #schedule_function(func=rebalance,
    #                  date_rule=date_rules.week_start(days_offset=4),
    #                  time_rule=time_rules.market_close(minutes=95),
    #                  half_days=True)
#    schedule_function(func=rebalance,
#                      date_rule=date_rules.week_start(days_offset=3),
#                      time_rule=time_rules.market_close(minutes=95),
...
#                      time_rule=time_rules.market_close(minutes=95),
#                      half_days=True)
    ###orig:probably only for minutely or live minutely trading
    ###cancel_open_orders = lambda context, data: context.order_mngr.cancel_open_orders(data)
...
def handle_data(context, data):
    ...
    if AlgoParms['IS_LIVE']:
        context.port_pnl_df, context.orders_done_today = find_latest_trade(daily_port_date_traded_file, context)
    else:
        context.orders_done_today = False
...
def before_trading_start(context, data):
    ...
    ## Fill Forward needed for yahoo_direct 2020-12-06: Ivanti aquires MOBL and takes it private.   
    #bundle_name = 'yahoo_direct' #'alpaca_api'
    #bundle_data = bundles.load(bundle_name)   
    #price_dta = data.history(symbol('MOBL'), fields='volume', bar_count=2, frequency='1d')
    ...
    ####context.exposure.long_actual_dist.name="long_actual_dist"
    ####context.exposure.short_actual_dist.name="short_actual_dist"
    #dist_df = context.strategies[0].fact_trade_dist
    ####dist_df.update(context.exposure.long_actual_dist)
    ####dist_df.fact_trade_dist.update(context.exposure.short_actual_dist)
    #log.debug( 'Output: Factor and trade distribution.csv: Time:US/Eastern {}'.format(current_time))
...
def rebalance(context, data):
    ...
    if AlgoParms['IS_LIVE']:
        if ((not context.ORDERS_DONE) 
             ...
             and (not context.orders_done_today)):
            ...
            pp.pprint("IB-account: BeforeOrders:{} {}".format(time_now, brk_acct_ao))
    ...
    order_verif_df = pd.DataFrame(0, index= context.my_etfs_sym, columns=['prev_pos','new_pos','all_orders'])
